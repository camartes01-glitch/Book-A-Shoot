export default async function handler(req, res) {
  ...
  try {
    ...
    const cleanName = String(name || 'Book A Shoot User').trim();
    const cleanEmail = String(email || 'anonymous@bookashoot.online').trim();
    ...
    return res.status(200).json({ success: true, message: 'Feedback dispatched to info@bookashoot.online', results: dispatchResults });
  } catch (err) {
