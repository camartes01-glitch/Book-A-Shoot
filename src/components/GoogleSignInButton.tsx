import React from "react";
import {
  ActivityIndicator,
  Pressable,
  StyleSheet,
  Text,
  View,
  type StyleProp,
  type ViewStyle,
} from "react-native";
import Svg, { Path } from "react-native-svg";
import { colors, radiusSm, touchTarget } from "@/src/constants/theme";
import { selectionFeedback } from "@/src/utils/haptics";

/** Official Google Multi-Color "G" Logo */
export function GoogleGLogo({ size = 20 }: { size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24">
      <Path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        fill="#4285F4"
      />
      <Path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <Path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
        fill="#FBBC05"
      />
      <Path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
        fill="#EA4335"
      />
    </Svg>
  );
}

export interface GoogleSignInButtonProps {
  onPress: () => void;
  disabled?: boolean;
  loading?: boolean;
  label?: string;
  style?: StyleProp<ViewStyle>;
}

/**
 * Standard Google Sign-In Button compliant with Google Identity Services guidelines:
 * - White background (#FFFFFF)
 * - Subtle neutral border (#DADCE0)
 * - Official multi-color Google G logo
 * - Google brand font color (#3C4043)
 */
export function GoogleSignInButton({
  onPress,
  disabled = false,
  loading = false,
  label = "Continue with Google",
  style,
}: GoogleSignInButtonProps) {
  const inactive = disabled || loading;

  return (
    <Pressable
      onPress={() => {
        void selectionFeedback();
        onPress();
      }}
      disabled={inactive}
      accessibilityRole="button"
      accessibilityLabel={label}
      accessibilityState={{ disabled: inactive, busy: loading }}
      style={({ pressed }) => [
        styles.button,
        pressed && styles.buttonPressed,
        inactive && styles.buttonDisabled,
        style,
      ]}
    >
      {loading ? (
        <View style={styles.contentRow}>
          <ActivityIndicator size="small" color="#4285F4" style={{ marginRight: 8 }} />
          <Text style={styles.label}>Connecting to Google...</Text>
        </View>
      ) : (
        <View style={styles.contentRow}>
          <View style={styles.iconContainer}>
            <GoogleGLogo size={19} />
          </View>
          <Text style={styles.label}>{label}</Text>
        </View>
      )}
    </Pressable>
  );
}

const styles = StyleSheet.create({
  button: {
    minHeight: touchTarget,
    height: 48,
    borderRadius: radiusSm,
    backgroundColor: "#FFFFFF",
    borderWidth: 1,
    borderColor: "#DADCE0",
    paddingHorizontal: 16,
    alignItems: "center",
    justifyContent: "center",
    boxShadow: "0px 1px 2px rgba(0, 0, 0, 0.08)",
    elevation: 1,
  },
  buttonPressed: {
    backgroundColor: "#F8F9FA",
    borderColor: "#C6C9CD",
    transform: [{ scale: 0.995 }],
  },
  buttonDisabled: {
    opacity: 0.65,
  },
  contentRow: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  },
  iconContainer: {
    marginRight: 12,
    alignItems: "center",
    justifyContent: "center",
  },
  label: {
    color: "#3C4043",
    fontSize: 15,
    fontWeight: "600",
    letterSpacing: 0.2,
  },
});
